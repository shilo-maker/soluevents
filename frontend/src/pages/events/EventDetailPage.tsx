...
export default function EventDetailPage() {
  ...
  const [soluflowCopied, setSoluflowCopied] = useState(false)
                ...
                {showPlanFurther && (
                  <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    {!event.rider_details && (() => {
                      ...
                      const hasWorshipTeam = event.event_teams?.some((team: any) =>
                        (team.name === worshipName || team.name?.toLowerCase().includes('worship')) && team.members?.some((m: any) => m.name?.trim())
                      )
                    ...
                    {event.flow_service_id && linkedService?.code && (
                      <div className="relative group">
                        ...
                        <div className="absolute right-0 bottom-full pb-1 hidden group-hover:flex flex-col z-10 min-w-[140px]"><div className="bg-white rounded-lg shadow-lg border border-gray-200 py-1">
                          ...
                          >
                            {soluflowCopied ? <Check className="w-3.5 h-3.5 text-green-600" /> : <Copy className="w-3.5 h-3.5" />}
                            {soluflowCopied ? t('common.copied') : t('common.copyLink')}
                          </button>
                          <a
                            href={`https://wa.me/?text=${encodeURIComponent(
                              [
                                ...
                                event.location_name || '',
                                '',
                                `https://soluflow.app/guest/${linkedService.code}`,
                              ].filter(Boolean).join('\n')
                            )}`}
                            target="_blank"
                            rel="noopener noreferrer"
